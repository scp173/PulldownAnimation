...
@property (nonatomic,assign) float nowheight;
...
- (void)configShapeLayer
{
    ...
    _shapeLayer.fillColor = [UIColor colorWithRed:57/255.0 green:156/255.0 blue:141/255.0 alpha:1.0].CGColor;
...
- (void)configCurveView
{
    ...
    self.curveY = ROUNDHEIGHT;                 // r5点y坐标
...
- (void)handlePanAction:(UIPanGestureRecognizer *)pan
{
    if(!_isAnimating)
    {
        if(pan.state == UIGestureRecognizerStateChanged)
        {
            ...
            // 这部分代码使r5红点跟着手势走
            _mHeight = point.y*0.7 + ROUNDHEIGHT;
            //self.curveX = SYS_DEVICE_WIDTH/2.0 + point.x;
            self.curveY = _mHeight > ROUNDHEIGHT ? _mHeight : ROUNDHEIGHT;
                 ...
                 pan.state == UIGestureRecognizerStateFailed)
        {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                _curveView.frame = CGRectMake(SYS_DEVICE_WIDTH/2.0, ROUNDHEIGHT, 3, 3);
